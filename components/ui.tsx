...
export const Card: React.FC<CardProps> = ({ children, className, ...props }) => (
  <div className={cn("glass-card rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden transition-all duration-300", className)} {...props}>
    {children}
  </div>
);
...
export const Button = ({ children, className, variant = 'primary', size = 'md', ...props }: ButtonProps) => {
  ...
  return (
    <button
      className={cn("font-medium rounded-md transition-all duration-200 active:scale-95 flex items-center justify-center gap-2 disabled:opacity-50 disabled:pointer-events-none shadow-sm", variants[variant], sizes[size], className)}
      {...props}
    >
      {children}
    </button>
...
export const Input = ({ className, ...props }: InputProps) => (
  <input
    className={cn("w-full px-3 py-1.5 rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-sm text-slate-900 dark:text-white focus:ring-2 focus:ring-cyan-500 focus:border-transparent outline-none transition-all placeholder:text-slate-400 dark:placeholder:text-slate-500 shadow-sm", className)}
    {...props}
  />
);
...
export const Select = ({ className, children, ...props }: SelectProps) => (
  <select
    className={cn("w-full px-3 py-1.5 rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-sm text-slate-900 dark:text-white focus:ring-2 focus:ring-cyan-500 focus:border-transparent outline-none transition-all appearance-none cursor-pointer shadow-sm", className)}
    {...props}
  >
    {children}
  </select>
...
export const Label = ({ children, className, ...props }: LabelProps) => (
  <label className={cn("block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1", className)} {...props}>
    {children}
  </label>
);
...
export const Dialog = ({ isOpen, onClose, title, children }: DialogProps) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-slate-900 rounded-lg shadow-2xl w-full max-w-lg p-5 animate-in fade-in zoom-in-95 duration-200 border border-slate-200 dark:border-slate-800">

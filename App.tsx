...
import { 
  ...
  CheckCircle2, Clock, AlertTriangle, Moon, Sun, Menu, X, ChevronDown, LayoutDashboard
} from 'lucide-react';
...
const Sidebar = ({ isOpen, setIsOpen }: { isOpen: boolean, setIsOpen: (v: boolean) => void }) => {
  const { currentUser, currentView, setCurrentView, isDarkMode, toggleTheme, switchUser } = useAppContext();
        ...
        <div className="p-6 flex items-center gap-3 border-b border-slate-100 dark:border-slate-800">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 font-medium">
              {currentUser.user_type === 'funcionario' ? '🔧 Painel da Marina' : '⚓ Gestão Náutica'}
            </p>
          </div>
        </div>
       ...
       <Card className="p-8">
          <div className="flex flex-col md:flex-row items-center gap-8 mb-8">
            ...
            <div className="text-center md:text-left">
              ...
              <p className="text-slate-500 capitalize">{currentUser.user_type}</p>
              <p className="text-xs text-slate-400 mt-1">Membro desde {new Date(currentUser.created_at).getFullYear()}</p>
            </div>
          </div>
          ...
          <form className="space-y-4">
            ...
            <div>
              <Label>E-mail</Label>
              <Input defaultValue={currentUser.email} disabled className="bg-slate-100 dark:bg-slate-800 opacity-70 cursor-not-allowed" />
            </div>
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [currentUser, setCurrentUser] = useState<User>(CURRENT_USER_CLIENT);
  ...
  // Logic
  const switchUser = () => {
    setCurrentUser(prev => prev.user_type === 'cliente' ? CURRENT_USER_EMPLOYEE : CURRENT_USER_CLIENT);
    setCurrentView('dashboard'); // Reset view on switch
    addNotification(`Perfil alterado para ${currentUser.user_type === 'cliente' ? 'Funcionário' : 'Cliente'}`);
  };

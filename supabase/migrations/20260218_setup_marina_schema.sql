-- Migration to add Marina Boat tables to EspecialSenior project

-- 1. Update profiles table
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS user_type text DEFAULT 'cliente';
-- Note: existing 'role' column in EspecialSenior might conflict logically but safe to add user_type for Marina app.
...
-- 6. Create RLS Policies (Permissive for Demo/MVP)
-- Boats: Owners can view/edit their boats. Admins (user_type='funcionario') can view all.
...
CREATE POLICY "Users can insert own boats" ON public.boats
    FOR INSERT WITH CHECK (auth.uid() = owner_id);

CREATE POLICY "Users can update own boats" ON public.boats
    FOR UPDATE USING (auth.uid() = owner_id);

CREATE POLICY "Users can delete own boats" ON public.boats
...
-- Service Requests: Users view own, Admins view all
...
-- Allow public insert for "Client Registration" flow (which inserts into profiles)
-- Note: profiles usually handled by trigger on auth.users, but App.tsx does direct insert for "Add Client" by Admin.
-- Adjusting profiles policy for Admin access.
CREATE POLICY "Admins manage profiles" ON public.profiles
    FOR ALL USING ((SELECT user_type FROM public.profiles WHERE id = auth.uid()) = 'funcionario');

-- For the sake of the demo app which might allow anon signup or client registration:
